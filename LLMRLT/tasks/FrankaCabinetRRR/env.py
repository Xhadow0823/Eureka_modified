# FrankaCabinet Task R3D version
import numpy as np
import os
import torch
...
class FrankaCabinetRRR(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        _, self.reset_buf[:], _ = compute_reward_R3D(  # this is by human, only get the reset buffer
    ...
    def post_physics_step(self):
        ...
        # ========== SR ==========
        open_far_enough = (self.cabinet_dof_pos[:, 3] > 0.2)
        self.has_success |= open_far_enough
        self.extras.update({
            "r/SR": self.has_success.to(torch.float).mean()
        })
        # ========== SR ==========
...
def compute_reward_R3D(
    ...
    'Cabinet task reward function in R3D by human'
