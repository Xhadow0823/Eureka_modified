...
class FrankaLift2(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        # call for 3xstates
        # self.rew_buf[:], self.reward_components = compute_reward(
        #     self.states["cubeA_pos"], self.states["eef_pos"], self.states["cubeA_pos_relative"], self.actions[:,-1], self.states["FSM"]
        # )
        # call for 4xstates
        # base_pos = torch.tensor([-0.45, 0, 1.125], device=self.device)
        # self.rew_buf[:], self.reward_components = compute_reward2(
        #     self.states["cubeA_pos"], self.states["cubeA_quat"], self.states["eef_pos"], self.states["eef_quat"], self.states["cubeA_pos_relative"], self.actions[:,-1], base_pos, self.states["FSM"]
        # )
...
# this is human expert version reward function
@torch.jit.script
