...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.velocity, self.timestep)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, velocity: torch.Tensor, timestep: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract the forward velocity component of the ant
    ...
    # Reward transformation; with temperature parameter for each transformed reward component
    forward_temperature = 0.1
    transformed_forward_reward = torch.exp(forward_temperature * forward_reward) - 1

    # Sum the reward components
    total_reward = transformed_forward_reward + alive_bonus

    # Store individual reward components in a dictionary for analysis
    reward_components = {
