...
def compute_reward(dof_pos: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    pole_velocity = dof_vel[:, 1]

    # Parameters and constants
    angle_threshold = torch.tensor(12, device=dof_pos.device).mul_(3.1415 / 180)
    position_threshold = torch.tensor(2.4, device=dof_pos.device)
    angle_temperature = torch.tensor(0.5, device=dof_pos.device)
    position_temperature = torch.tensor(0.5, device=dof_pos.device)

    # Calculate reward components
    angle_reward = torch.exp(-angle_temperature * torch.abs(pole_angle))
    position_reward = torch.exp(-position_temperature * torch.abs(cart_position))

    # Calculate total reward
    total_reward = torch.min(angle_reward, position_reward)
