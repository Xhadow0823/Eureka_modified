...
class FrankaCubeStackGPT(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(10.0, 10.0, 2.4)
            cam_target = gymapi.Vec3(2.0, 2.0, 1.0)
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.cubeA_pos, self.cubeA_to_cubeB_pos, self.eef_pos, self.cubeA_quat)
...
@torch.jit.script
def compute_reward(cubeA_pos: torch.Tensor, cubeA_to_cubeB_pos: torch.Tensor,
                   eef_pos: torch.Tensor, cubeA_quat: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    
    goal_pos = cubeA_pos + cubeA_to_cubeB_pos  # calculate the position of the top of the stacked cube
    distance = torch.norm(goal_pos - eef_pos)  # calculate the distance between the end-effector and the top of the stacked cube

    # Reward for reaching the goal position
    reach_goal_reward_temperature = 1.0
    reach_goal_reward = torch.exp(-reach_goal_reward_temperature * distance)

    # Reward for maintaining the cube orientation
    cubeA_quat_target = torch.tensor([1, 0, 0, 0], device=cubeA_quat.device)  # assume the desired orientation is a unit quaternion
    orientation_error = torch.norm(cubeA_quat - cubeA_quat_target)
    orientation_reward_temperature = 1.0
    orientation_reward = torch.exp(-orientation_reward_temperature * orientation_error)

    # Combine rewards
    total_reward = reach_goal_reward + orientation_reward
