...
class FrankaReachGPT(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # dimensions
        # obs include: cubeA_pose (7) + eef_pose (7)
        self.cfg["env"]["numObservations"] =14 if self.control_type == "osc" else 26  # 0123 modified  obs is 3+4(cubeA_pose) now
        # actions include: delta EEF if OSC (6) 
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.eef_pos, self.cubeA_pos)
    ...
    def reset_idx(self, env_ids):
        ...
        # progress_buf is current steps of this iteration
...
from torch import Tensor
@torch.jit.script
def compute_reward(eef_pos: torch.Tensor, cubeA_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Distance between end effector and cubeA
    distance = torch.norm(eef_pos - cubeA_pos, dim=-1)

    # Normalize reward value to be in range [0, 1]
    max_distance = torch.tensor(2.0).to(distance.device)
    normalized_distance = torch.clamp(distance / max_distance, 0, 1)

    # Reward function
    distance_temperature = torch.tensor(70.0).to(distance.device)
    distance_reward = torch.exp(-distance_temperature * normalized_distance)

    # Time penalty
    time_penalty_scale = torch.tensor(1.0).to(distance.device)
    timestep_penalty = torch.tensor(0.0075).to(distance.device)
    time_penalty = -timestep_penalty * time_penalty_scale

    # Successful touch reward
    touch_reward = torch.zeros_like(distance)
    touch_threshold = torch.tensor(0.03).to(distance.device)
    touch_reward_value = torch.tensor(4.0).to(distance.device)
    mask = torch.le(distance, touch_threshold)
    touch_reward = torch.where(mask, touch_reward_value, touch_reward)

    # Total reward
    total_reward = distance_reward + time_penalty + touch_reward

    return total_reward, {'distance': -distance, 'time_penalty': time_penalty, 'touch_reward': touch_reward}
